...
with project_client.agents.create_stream(
    ...
    for event_type, event_data, func_return in stream:
        print(
            f"Streaming receive Event Type: {event_type}, Event Data in: {type(event_data)}, Event Function return: {func_return}"
        )
```

<!-- END SNIPPET -->

In this subclass, we override the event functions and return messages in strings.  The return type is defined in the inheritent in Generic.  The return messages or objects are accessibled in the iteration.  
